...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    // Formatting the auth header value, prepend Bearer String with the OAI_TOKEN
    ...
    loop {
        ...
        // Use flush() when you need to be sure that all your data from buffer is written

        // Flushing a stream ensures that all data that has been written to that stream is output,
        // including clearing any that may have been buffered.

        // A buffer is a temporary storage area for data that is being sent from one place to another.
        // It is used to store data while it is being moved from one place to another. It is a temporary storage area.
        ...
        // Aggregate the data buffers from a body asynchronously (hence await).
        // Waiting for all the chunks of data to come back and pull a body out of that.
        ...
        // Pinting the response, choices is coming from the OpenAI API response struct, so we are accessing the choices vector and getting the first one
